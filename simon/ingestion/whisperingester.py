...
class WhisperIngester:
    def __init__(self, context:AgentContext, model_name="base"):
    ...
    def ingest_file(self, path, title=None, source=None):
        ...
        """
        L.info(f"Whisper Ingesting {path}...")
        transcript = self.model.transcribe(path)["text"].strip()
        L.debug(f"Transcribed {path}.")
        doc = parse_text(transcript, title, source)
        L.debug(f"Successfully chunked {path}.")
